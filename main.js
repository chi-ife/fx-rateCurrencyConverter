...
// fetching the exchange rate and update to DOM
const calculate= () =>{
    const currency_one = currencyE1_one.value;
    const currency_two = currencyE1_two.value;

    window.fetch(`https://v6.exchangerate-api.com/v6/d4169bc2844c62ccfa543887/latest/${currency_one}`).then(res => res.json()).then(data => {
        console.log(data); //to check rates exist in data.
